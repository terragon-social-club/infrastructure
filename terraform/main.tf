...
module "Logstash" {
  ...
  node_count = var.cluster_makeup.logstash_workers
  provision = var.cluster_makeup.logstash_workers > 0
  name = "logstash"
  size = "c-2"
  domain_id = "terragon.us"
  keys = [
...
module "ElasticSearch" {
  source = "./modules/salt-minion"
  node_count = var.cluster_makeup.elasticsearch_workers
...
module "CouchDB" {
  source = "./modules/salt-minion"
  node_count = var.cluster_makeup.couchdb_replicas
  provision = var.cluster_makeup.couchdb_replicas > 0
  name = "couchdb"
  size = "s-2vcpu-2gb"
  domain_id = "terragon.us"
  keys = [
...
module "HAProxyCouchDB" {
  source = "./modules/salt-minion"
  node_count = (var.cluster_makeup.couchdb_replicas > 0) ? 1 : 0
  provision = var.cluster_makeup.couchdb_replicas > 0
