...
resource "digitalocean_droplet" "salt_minion" {
  ...
  ssh_keys = var.keys
  ipv6 = false
  tags = [replace(var.salt_master_public_ip_address, ".", "-"), var.autogenerated_ssh_private_key, var.name, var.alpha[count.index]] // This is a hack to ensure no outside references to avoid warning
