...
resource "digitalocean_droplet" "salt_minion" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      host = replace(element(self.digitalocean_droplet.salt_minion.*.tags[0], count.index), "-", ".")) // Line 65 lets me write this to avoid warning
      private_key = var.autogenerated_ssh_private_key // It has to let me use a private_key wtf
      user = "root"
      type = "ssh"
      timeout = "2m"
    }
    ...
    inline = [
      "salt-key -d ${element(digitalocean_droplet.salt_minion.*.tags[1], count.index)} -${element(digitalocean_droplet.salt_minion.*.tags[2], count.index)} -y"// Line 65 lets me write this to avoid warning
    ]
