...
resource "digitalocean_droplet" "salt_minion" {
  ...
  ssh_keys = var.keys
  ipv6 = false
  tags = [var.salt_master_public_ip_address, var.autogenerated_ssh_private_key, var.name, var.alpha[count.index]] // This is a hack to ensure no outside references to avoid warning
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "salt-key -d ${self.tags[2]} -${self.tags[3]} -y"// Line 65 lets me write this to avoid warning
    ]
